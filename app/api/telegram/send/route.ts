...
function statusForSendError(error?: string) {
  ...
  if (error === 'Only telegram channel supported') return 400
  if (error === 'No Telegram bot token configured for this store') return 500
  return 502
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await sendTelegramMessage(supabase, {
      conversationId,
