...
export async function preprocessMessage(input: SuggestReplyInput): Promise<PreprocessingResult> {
  ...
  const system = [
    ...
    'The intent field must be exactly one of these lowercase tokens, with no synonyms, paraphrases, or variations: order_status, shipping, product_question, returns, refund, dispute, pricing, availability, other.',
    'Intent guidance: use order_status for any question about an existing order — where it is, whether it has shipped/been dispatched/sent out, its delivery progress, or its tracking number. Use shipping only for pre-purchase questions about delivery options, cost, or timeframes. Use other only when none of the listed intents apply.',
